...
app = FastAPI(title="Kenshin Anime API v7")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"status": "Kenshin Anime API v7 running!"}
...
async def get_by_title(title: str):
    doc = await db.anime.find_one(
        {"title": {"$regex": f"^{re.escape(title)}$", "$options": "i"}, "visible": True},
        {"_id": 0}
    )
    return doc or {}
...
async def add_demand(data: DemandItem):
    ...
    return {"ok": True, "msg": f"Demand submitted!"}
...
async def admin_add(data: AnimeItem, token: str):
    auth(token)
    d = data.dict()
    if not d.get("dl_link1") and d.get("tg_link"):
        d["dl_link1"] = d["tg_link"]
    ex = await db.anime.find_one({"title": {"$regex": f"^{re.escape(data.title)}$", "$options": "i"}})
    if ex:
        await db.anime.update_one({"_id": ex["_id"]}, {"$set": d})
        return {"ok": True, "msg": f"'{data.title}' updated!"}
...
async def admin_get_episodes(anime_title: str, token: str):
    ...
    eps = await db.episodes.find(
        {"anime_title": {"$regex": f"^{re.escape(anime_title)}$", "$options": "i"}},
        {"_id": 1, "season": 1, "number": 1, "title": 1, "url": 1,
         "thumbnail": 1, "duration": 1, "isNew": 1, "audioTracks": 1, "subtitles": 1}
    ).sort([("season", 1), ("number", 1)]).to_list(10000)
...
async def admin_notifs(token: str):
    ...
    items = await db.notifications.find(
        {}, {"_id": 1, "message": 1, "anime_title": 1, "active": 1, "type": 1, "created_at": 1}
    ).to_list(100)
    return [fix_id(i) for i in items]
...
async def admin_comments(token: str):
    auth(token)
    items = await db.comments.find(
        {}, {"_id": 1, "anime_title": 1, "username": 1, "comment": 1, "created_at": 1}
    ).to_list(500)
    return [fix_id(i) for i in items]
...
async def admin_demands(token: str):
    ...
    items = await db.demands.find(
        {}, {"_id": 1, "title": 1, "category": 1, "username": 1,
             "reason": 1, "votes": 1, "status": 1, "created_at": 1}
    ).to_list(500)
    return [fix_id(i) for i in items]
